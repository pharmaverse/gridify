...
elements for pharma layouts. It defines the layout with specified
margins, global graphical parameters, and height adjustment. The layout
includes cells for headers, titles, footers, and optional elements like
watermarks.
...
- margin:
  ...
  A [`grid::unit`](https://rdrr.io/r/grid/unit.html) object defining the
  margins of the layout (top, right, bottom, left) in inches. Default is
  `grid::unit(c(1, 1, 1, 1), "inches")`.

- global_gpar:

  A list specifying global graphical parameters to change in the layout.
  Default is NULL, however the defaults in the layout are:
  `fontfamily = "Serif", fontsize = 9 and lineheight = 0.95`, which can
  be overwritten alongside other graphical parameters found by
...
A `gridifyLayout` object that defines the general structure and
parameters for a pharma layout.

## Details

This function is primarily used internally by other layout functions
such as
