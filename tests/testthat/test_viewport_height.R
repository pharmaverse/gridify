...
test_that("is_flexible_grob detects gTrees carrying a childrenvp", {
  # Shape produced internally by gridGraphics::grid.echo() for the
  # `formula` input type; constructed directly to avoid the dependency.
  rec <- grid::gTree(
    children = grid::gList(grid::rectGrob()),
    childrenvp = grid::viewport()
  )
  expect_true(is_flexible_grob(rec))
})
...
test_that("is_flexible_grob returns FALSE for a gTree without childrenvp", {
  g <- grid::gTree(children = grid::gList(grid::rectGrob()))
...
test_that("object_viewport_height_expr uses npc height for flexible grobs", {
  ...
  e <- object_viewport_height_expr(rec, vjust = 0, height = 0.5)
  inner <- e[[2]]
  expect_identical(inner[[1]], quote(grid::unit))
...
test_that("object_viewport_height_expr always floors at 1 inch", {
  flex <- grid::gTree(
    children = grid::gList(grid::rectGrob()),
    childrenvp = grid::viewport()
  )
  for (case in list(
    ...
    expect_identical(floor_arg[[3]], "inch")
  }
})
