...
By default `export_to()` also writes a JSON sidecar file next to the output
(e.g. `output.pdf.json`) containing the text values you set with `set_cell()`.
This makes it easy to track which header, footer or watermark text was used to
produce a given figure without parsing the file.

```{r, eval = FALSE}
# Default: writes both output.pdf and output.pdf.json
export_to(gridify_obj, to = "output.pdf")

# Disable the sidecar
export_to(gridify_obj, to = "output.pdf", metadata = FALSE)
