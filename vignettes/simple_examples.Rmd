...
`export_to()` can optionally record the text values you set with `set_cell()`
alongside the output, making it easy to track which header, footer or
watermark text was used to produce a given figure without parsing the file.
...
`metadata = "sidecar"` but no `set_cell()` text values, any existing sidecar for
that output is removed. This prevents an older `output.pdf.json` from being
