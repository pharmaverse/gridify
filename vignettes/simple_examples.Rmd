...
The feature is opt-in via the `metadata` argument.
...
# PDF only: embed the same JSON payload in the PDF /Title metadata
# (no sidecar file is written in this mode)
export_to(gridify_obj, to = "output.pdf", metadata = "embed")
```
