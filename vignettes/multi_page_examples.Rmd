...
each output file. The exact content depends on how the output is structured:

- **Single multi-page PDF** (a list of objects exported to one `.pdf` path) —
  one sidecar `<file>.pdf.json` containing a JSON array with one object per
  page, listing the `set_cell()` text values for that page.
- **Multiple separate files** (a list of objects exported to a vector of paths,
  ...
  `fig2.pdf.json`, …), each containing a single JSON object.
