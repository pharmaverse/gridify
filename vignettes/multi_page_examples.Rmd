...
pages <- paginate_table(
  ...
  fill_last_page = TRUE,  # Optional: fills last page with empty rows for consistent positioning
  page_col = NULL         # Optional: set to NULL to avoid adding a page_number column
)
...
#### Manual Splitting (Alternative Method)

Alternatively, you can split the table manually if you need more control:
...
pages <- split(df, df$page_number)
```

### OPTIONAL - Fill last table with empty rows

When using `paginate_table()`, simply set `fill_last_page = TRUE` (as shown above).

For manual splitting, if the last table doesn't meet the `rows_per_page` specification, it will appear as if it was placed in the middle of the page.
